	...
	public class ArcGISMapComponentEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			if (showSpatialReferenceCategory)
			{
				...
				if (EditorGUI.EndChangeCheck())
				{
					mapComponent.UseCustomMapSpatialReference = useCustomMapSpatialReferenceProp.boolValue;
				}
			...
			if (showBasemapCategory)
			{
				...
				if (updateBasemap)
				{
					...
					basemapAuthenticationTypeProp.serializedObject.ApplyModifiedProperties();
			...
			if (EditorGUI.EndChangeCheck())
			{
				mapElevationProp.serializedObject.ApplyModifiedProperties();
			...
			if (mapTypeProp.enumNames[mapTypeProp.enumValueIndex] == "Local")
			{
				...
				if (enableExtentProp.boolValue)
				{
					...
					if (showExtentCategory)
					{
						...
						if (!mapComponent.Extent.UseOriginAsCenter)
						{
			...
			showAuthenticationCategory = EditorGUILayout.BeginFoldoutHeaderGroup(showAuthenticationCategory, "AuthenticationType");
